...
func Entries(node *yaml.Node) ([]MapEntry, error) {
	if err := validateKind(node, yaml.MappingNode); err != nil {
		return nil, err
	}
...
func Elements(node *yaml.Node) ([]*yaml.Node, error) {
	if err := validateKind(node, yaml.SequenceNode); err != nil {
		return nil, err
	}
...
func GetString(node *yaml.Node, key string) (string, error) {
	...
	if err := validateKind(val, yaml.ScalarNode); err != nil {
...
func SetScalar(node *yaml.Node, key, value string) error {
	...
	for i := 0; i+1 < len(node.Content); i += 2 {
		if node.Content[i].Value == key {
			if err := validateKind(node.Content[i+1], yaml.ScalarNode); err != nil {
				return fmt.Errorf("set %q: %w", key, err)
			}
	...
	// A comment trailing the mapping is parsed as the foot comment of the last
	// entry. Appending after it makes the encoder separate the comment from the
	// new entry with a blank line, so move the comment along to keep it trailing
	// the mapping.
	if n := len(node.Content); n >= 2 {
		for _, last := range []*yaml.Node{node.Content[n-1], node.Content[n-2]} {
			if last.FootComment != "" {
				keyNode.FootComment = last.FootComment
				last.FootComment = ""
				break
			}
		}
...
func validateKind(node *yaml.Node, kind yaml.Kind) error {
	if node == nil {
