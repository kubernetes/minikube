...
var (
	crictlSchema = map[string]update.Item{
		...
		"deploy/kicbase/Dockerfile": {
			Replace: map[string]string{
				`ARG CRICTL_VERSION=.*`: `ARG CRICTL_VERSION="{{.Version}}"`,
			},
		},
	}
)

type CrictlData struct {
	Version string
	Commit  string
}

func detectCrictl() (CrictlData, error) {
	...
	return CrictlData{Version: stable.Tag}, err
}

func (data CrictlData) updateCrictl() {

	update.Apply(crictlSchema, data)

	if err := updateCrictlHashFile(data.Version, "arm64", "aarch64/package/crictl-bin-aarch64"); err != nil {
		klog.Fatalf("failed to update hash files: %v", err)
	}
	if err := updateCrictlHashFile(data.Version, "amd64", "x86_64/package/crictl-bin"); err != nil {
		klog.Fatalf("failed to update hash files: %v", err)
	}

}

func updateCrictlHashFile(version, arch, packagePath string) error {
