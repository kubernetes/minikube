...
type QemuLinuxDockerArm64IntegrationTester struct {
}

// Run implements MiniTestTester.
func (k *QemuLinuxDockerArm64IntegrationTester) Run(runner MiniTestRunner) error {
	...
	if testErr = runner.Execute(fmt.Sprintf("cd minikube && PULL_NUMBER=\"%s\" ./hack/prow/integration_qemu_docker_linux_arm64.sh", pr)); testErr != nil {
