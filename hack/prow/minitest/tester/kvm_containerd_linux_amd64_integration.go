...
var _ MiniTestTester = &KVMContainerdLinuxAmd64IntegrationTester{}

// this runs the integration tests with kvm2 driver and a containerd container runtime.
...
func (k *KVMContainerdLinuxAmd64IntegrationTester) Run(runner MiniTestRunner) error {
	...
	if err := runner.SyncToRemote(".", "~/minikube", []string{".cache"}); err != nil {
		klog.Errorf("failed to sync file in containerd deployer: %v", err)
	}
	pr := os.Getenv("PULL_NUMBER")

	var testErr error
	// install containerd and libvirtd first then run the test in a new shell
	if err := runner.Execute("cd minikube && ./hack/prow/integration_kvm_containerd_linux_x86-64_pre.sh"); err != nil {
