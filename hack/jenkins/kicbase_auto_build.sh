...
if ! CIBUILD=yes make push-kic-base-image 2>&1 | tee kic-logs.txt; then
  ec=${PIPESTATUS[0]:-1}                       # bash-only; exit of `make`, not `tee`
  if [[ ${release:-false} != "true" ]]; then   # default release to false if unset
