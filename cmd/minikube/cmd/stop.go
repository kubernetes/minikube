...
func runStop(cmd *cobra.Command, args []string) {
	...
	if stoppedNodes > 0 {
		out.Step(style.Stopped, false, `{{.count}} nodes stopped.`, out.V{"count": stoppedNodes})
	}
}
...
func stop(api libmachine.API, machineName string) bool {
	...
	tryStop := func() (err error) {
		...
		case mcnerror.ErrHostDoesNotExist:
			out.Step(style.Meh, false, `"{{.machineName}}" does not exist, nothing to stop`, out.V{"machineName": machineName})
			nonexistent = true
			return nil
		default:
			return err
