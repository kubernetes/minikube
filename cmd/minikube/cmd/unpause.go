...
var unpauseCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, n := range co.Config.Nodes {
			...
			out.Step(style.Pause, "Unpausing node {{.name}} ... ", false, out.V{"name": name})
		...
		if namespaces == nil {
			out.Step(style.Pause, "Unpaused {{.count}} containers", false, out.V{"count": len(ids)})
		} else {
			out.Step(style.Pause, "Unpaused {{.count}} containers in: {{.namespaces}}", false, out.V{"count": len(ids), "namespaces": strings.Join(namespaces, ", ")})
		}
	},
}
