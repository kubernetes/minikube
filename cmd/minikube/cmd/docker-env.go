...
func mustRestartDocker(name string, runner command.Runner) {
	if err := sysinit.New(runner).Restart("docker"); err != nil {
		exit.WithCodeT(exit.RuntimeError, `The Docker service within '{{.name}}' is not active`, out.V{"name": name})
	}
}
...
var dockerEnvCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if driver.NeedsPortForward(driverName) {
			...
			if err != nil {
				exit.WithCodeT(exit.LocalNetworkError, "Error getting port binding for '{{.driver_name}} driver: {{.error}}", out.V{"driver_name": driverName, "error": err})
			}
		}
