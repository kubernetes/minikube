...
var mountCmd = &cobra.Command{
	...
	minikube mount <source directory>:<target directory>   (example: "/host-home:/vm-home")`)
		...
		out.Step(style.Mounting, out.NoSpinner, "Mounting host path {{.sourcePath}} into VM as {{.destinationPath}} ...", out.V{"sourcePath": hostPath, "destinationPath": vmPath})
		...
		go func() {
			for sig := range c {
				out.Step(style.Unmount, out.NoSpinner, "Unmounting {{.path}} ...", out.V{"path": vmPath})
		...
		out.Step(style.Success, out.NoSpinner, "Successfully mounted {{.sourcePath}} to {{.destinationPath}}", out.V{"sourcePath": hostPath, "destinationPath": vmPath})
		out.Ln("")
		out.Step(style.Notice, out.NoSpinner, "NOTE: This process must stay alive for the mount to be accessible ...")
		wg.Wait()
	},
}
