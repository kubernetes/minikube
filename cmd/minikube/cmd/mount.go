...
var mountCmd = &cobra.Command{
	...
	minikube mount <source directory>:<target directory>   (example: "/host-home:/vm-home")`)
		...
		out.Step(style.Mounting, false, "Mounting host path {{.sourcePath}} into VM as {{.destinationPath}} ...", out.V{"sourcePath": hostPath, "destinationPath": vmPath})
		...
		if cfg.Type == nineP {
			wg.Add(1)
			go func() {
				out.Step(style.Fileserver, false, "Userspace file server: ")
		...
		go func() {
			for sig := range c {
				out.Step(style.Unmount, false, "Unmounting {{.path}} ...", out.V{"path": vmPath})
		...
		out.Step(style.Success, false, "Successfully mounted {{.sourcePath}} to {{.destinationPath}}", out.V{"sourcePath": hostPath, "destinationPath": vmPath})
		out.Ln("")
		out.Step(style.Notice, false, "NOTE: This process must stay alive for the mount to be accessible ...")
		wg.Wait()
	},
}
