...
You can add one by annotating a service with the label {{.labelName}}:{{.addonName}}`, out.V{"labelName": key, "addonName": addonName})
		...
		for i := range serviceList.Items {
			...
			if len(urlString) != 0 {
				out.Step(style.Celebrate, out.NoSpinner, "Opening Kubernetes service  {{.namespace_name}}/{{.service_name}} in default browser...", out.V{"namespace_name": namespace, "service_name": svc})
