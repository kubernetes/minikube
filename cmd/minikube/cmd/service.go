...
You may select another namespace by using 'minikube service {{.service}} -n <namespace>'. Or list out all the services using 'minikube service list'`, out.V{"service": svc, "namespace": namespace})
			...
			exit.WithError("SVC_NOT_FOUND", "Error opening service", err)
		}
...
func startKicServiceTunnel(svc, configName string) {
	...
	if err != nil {
		exit.WithError("K8S_CLIENT", "error creating clientset", err)
	}

	port, err := oci.ForwardedPort(oci.Docker, configName, 22)
	if err != nil {
		exit.WithError("DRV_PORT_FORWARD", "error getting ssh port", err)
	}
	...
	if err != nil {
		exit.WithError("DRV_TUNNEL_START", "error starting tunnel", err)
	}
	...
	if err != nil {
		exit.WithError("DRV_TUNNEL_STOP", "error stopping tunnel", err)
	}
}
