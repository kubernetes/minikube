...
func initMinikubeFlags() {
	...
	startCmd.Flags().String(kicBaseImage, "", "The base image to use for docker/podman drivers. Intended for local development.")
...
func generateClusterConfig(cmd *cobra.Command, existing *config.ClusterConfig, k8sVersion string, drvName string) (config.ClusterConfig, config.Node, error) {
	...
	if existing != nil {
		...
		if kicBaseImage == "" {
			kicBaseImage = kic.GetBaseImage(viper.GetString(containerRuntime))
		}
