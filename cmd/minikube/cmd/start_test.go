...
func TestMirrorCountry(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			config, _, err := generateClusterConfig(cmd, nil, k8sVersion, rtime, driver.Mock, &run.Options{})
			if err != nil {
...
func TestGenerateCfgFromFlagsHTTPProxyHandling(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			proxy.SetDockerEnv()
			config, _, err := generateClusterConfig(cmd, nil, k8sVersion, rtime, "none", &run.Options{})
			if err != nil {
				t.Fatalf("Got unexpected error %v during config generation", err)
			}
