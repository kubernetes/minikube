...
func bootstrapCluster(bs bootstrapper.Bootstrapper, r cruntime.Manager, runner bootstrapper.CommandRunner, kc cfg.KubernetesConfig, preexisting bool) {
	...
	if preexisting {
		...
		if err := bs.RestartCluster(kc); err != nil {
			reportProblemsAndExit("Error restarting cluster", err, logs.FindProblems(r, bs, runner))
		}
		return
	}
	...
	if err := bs.StartCluster(kc); err != nil {
		reportProblemsAndExit("Error starting cluster", err, logs.FindProblems(r, bs, runner))
	}
}
...
func validateCluster(bs bootstrapper.Bootstrapper, r cruntime.Manager, runner bootstrapper.CommandRunner, ip string) {
	...
	if err != nil {
		reportProblemsAndExit("kubelet checks failed", err, logs.FindProblems(r, bs, runner))
	}
	...
	if err != nil {
		reportProblemsAndExit("apiserver checks failed", err, logs.FindProblems(r, bs, runner))
	}
	console.OutLn("")
}
...
// reportErrAndExit is a shortcut for outputting an error, reporting it, and exiting.
...
func reportProblemsAndExit(msg string, err error, problems map[string][]string) {
	console.Fatal(msg+": %v", err)
	logs.OutputProblems(problems, 3)
	cmdutil.MaybeReportErrorAndExit(err)
	os.Exit(1)
}
