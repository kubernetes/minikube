...
func testFunctionalInternal(t *testing.T, k8sVersion string) {
	...
	t.Run("serial", func(t *testing.T) {
		tests := []struct {
			...
			{"StartWithCustomCerts", validateStartWithCustomCerts},
...
// only runs on GitHub Actions for amd64 linux, skipped on podman+crio and other combos where it hangs
func validateStartWithCustomCerts(ctx context.Context, t *testing.T, profile string) {
	defer PostMortemLogs(t, profile)
	if !runCorpProxy {
		t.Skip("skipping StartWithCustomCerts: only runs on GitHub Actions linux amd64")
	}
