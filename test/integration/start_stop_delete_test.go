		...
		for _, tc := range tests {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if strings.Contains(tc.name, "cni") {
					...
					if _, err := PodWait(ctx, t, profile, "default", "integration-test=busybox", Minutes(5)); err != nil {
						t.Fatalf("failed waiting for pod 'busybox' post-stop-start: %v", err)
					}
					if _, err := PodWait(ctx, t, profile, "kubernetes-dashboard", "k8s-app=kubernetes-dashboard", Minutes(6)); err != nil {
