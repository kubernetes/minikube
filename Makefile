...
html_report: ## Generate HTML  report out of the last ran integration test logs.
	...
	@if ! command -v gopogh >/dev/null 2>&1; then \
		...
		echo "installed gopogh!"; \
	fi
	@"$(shell go env GOPATH)/bin/gopogh" -in "./out/testout_$(COMMIT_SHORT).json" -out ./out/testout_$(COMMIT_SHORT).html -name "$(shell git rev-parse --abbrev-ref HEAD)" -pr "" -repo github.com/kubernetes/minikube/  -details "${COMMIT_SHORT}"
