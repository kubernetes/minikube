...
The add-on will create start and create secrets in each namespace that can be used for `imagePullSecrets` on a pod.  It also updates the `default` service account in each namespace to use these `imagePullSecrets`, which means any pod created without an explicit service account other than `default`
will use the image pull secrets automatically.
