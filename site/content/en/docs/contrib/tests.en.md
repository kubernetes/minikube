...
Steps:
- Get the minikube node labels by `kubectl get nodes --output=go-template --template='{{range $k, $v := (index .items 0).metadata.labels}}{{$k}} {{end}}'`
- Make sure `minikube.k8s.io/commit`, `minikube.k8s.io/version`, `minikube.k8s.io/updated_at`, `minikube.k8s.io/name` are in the node labels

#### validateLoadImage
makes sure that `minikube image load` works as expected

Steps:
- Pull the `busybox:1.33` Docker image
- Tag the image to `docker.io/library/busybox:load-{profile}`
- Try to load the new image into minikube with `minikube image load ...`
...
Steps:
- Pull the `busybox:1.32` Docker image
- Tag the image to `docker.io/library/busybox:load-{profile}`
- Try to load the new image into minikube with `minikube image load ...`
- Try to remove the image from minikube with `minikube image rm ...`
- Make sure the image was correctly removed
...
Steps:
- Try to list the images with `minikube image ls ...`
- Make sure the images are listed
...
Steps:
- Run `eval $(minikube docker-env)` to configure current environment to use minikube's Docker daemon, and `minikube status` to get the minikube status
...
- Run `minikube start --apiserver-port=8441 --wait=all` with the environment variable `HTTP_PROXY` set to the local HTTP proxy
- Later in `validateSoftStart` we will make sure `--apiserver-port` is not changed
- Make sure the environment variable `HTTP_PROXY` is respected by verifying the `minikube start` output
...
- Make sure it contains the current minikube profile name

#### validateSoftStart
validates that after minikube already started, a `minikube start` should not change the configs.

Steps:
- The test `validateStartWithProxy` should already started minikube, make sure the configured node port is `8441`
...
Steps:
- Set environemnt variable `LC_ALL=fr` to enable minikube translation to French
...
Steps:
- The test certificates should have been synced into minikube in the previous step `setupFileSync`
- Check the existence of the test certificates
- Make sure the test certificates are synced correctly
