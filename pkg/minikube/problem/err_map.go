...
// Program errors are issues with the minikube binary
var ProgramErrors = map[string]match{
	"INVALID_KUBERNETES_VERSION": {
		...
		ExitCode: exitcode.ProgramUsage,
	},
...
// ResourceErrors are failures due to resource constraints
var ResourceErrors = map[string]match{
	"KVM_OOM": {
		Regexp:   re(`cannot set up guest memory.*Cannot allocate memory`),
		Advice:   "Choose a smaller value for --memory, such as 2000",
		GOOS:     []string{"linux"},
		Issues:   []int{6366},
		ExitCode: exitcode.InsufficientMemory,
	},
	"OOM_KILL_SSH": {
...
`,
		Issues:   []int{9024},
		ExitCode: exitcode.InsufficientStorage,
	},
}	

// HostErrors are related to the host operating system or BIOS
var HostErrors = map[string]match{
	"VBOX_VTX_DISABLED": {
	...
	"KUBECONFIG_WRITE_FAIL": {
		...
		ExitCode: exitcode.HostPermissions,
	},
	"KUBECONFIG_DENIED": {
	...
	"DOCKER_CHROMEOS": {
			...
			Issues:   []int{6411},
			ExitCode: exitcode.HostUnsupported,
		},
	},
}

// ProviderErrors are failures relating to a driver provider
var ProviderErrors = map[string]match{
	// General
	"PRECREATE_EXIT_1": {
	...
	"DOCKER_PROVISION_STUCK_CONTAINER": {
		...
		Issues:   []int{8163},
		ExitCode: exitcode.ProviderTimeout,
	},
	...
	"HYPERKIT_NOT_FOUND": {
		...
		URL:      "https://minikube.sigs.k8s.io/docs/reference/drivers/hyperkit/",
		GOOS:     []string{"darwin"},
		ExitCode: exitcode.ProviderNotFound,
	},
	...
	"HYPERV_AS_ADMIN": {
		...
		Issues:   []int{4511},
		GOOS:     []string{"windows"},
		ExitCode: exitcode.ProviderPermission,
	},
	"HYPERV_NEEDS_ESC": {
	...
	"KVM_UNAVAILABLE": {
		...
		Issues:   []int{2991},
		GOOS:     []string{"linux"},
		ExitCode: exitcode.ProviderUnavailable,
	},
	...
	"KVM_ISO_PERMISSION": {
		...
		GOOS:     []string{"linux"},
		Issues:   []int{5950},
		ExitCode: exitcode.ProviderPermissions,
	},
	"KVM2_NETWORK_DEFINE_XML": {
		...
		Issues:   []int{4195},
		GOOS:     []string{"linux"},
		ExitCode: exitcode.ProviderConfig,
	},
	"KVM2_FAILED_MSR": {
	...
	"VBOX_DRV_NOT_LOADED": {
		...
		ExitCode: exitcode.ProviderNotRunning,
	},
	"VBOX_DEVICE_MISSING": {
	...
	"VBOX_HARDENING": {
		...
		GOOS:     []string{"windows"},
		ExitCode: exitcode.ProviderConflict,
	},
	"VBOX_NS_ERRROR": {
...
// DriverErrors are specific to a libmachine driver
var DriverErrors = map[string]match{
	// Generic VM driver
	"DRIVER_CORRUPT": {
	...
	"DRIVER_NOT_FOUND": {
		...
		Issues:   []int{5295},
		ExitCode: exitcode.DriverUnsupported,
	},
	...
	"CREATE_TIMEOUT": {
		...
		Issues:   []int{7072},
		ExitCode: exitcode.DriverTimeout,
	},
	"IMAGE_ARCH": {
	...
	"HYPERV_NO_VSWITCH": {
		...
		GOOS:     []string{"windows"},
		ExitCode: exitcode.DriverConfig,
	},
	...
	"HYPERV_POWERSHELL_NOT_FOUND": {
		...
		URL:      "https://docs.docker.com/machine/drivers/hyper-v/",
		GOOS:     []string{"windows"},
		ExitCode: exitcode.DriverUnavailable,
	},
	...
	"HYPERV_FILE_DELETE_FAILURE": {
		...
		Issues:   []int{6804},
		GOOS:     []string{"windows"},
		ExitCode: exitcode.DriverConflict,
	},
	...
	"KVM2_NOT_FOUND": {
		...
		URL:      "https://minikube.sigs.k8s.io/docs/reference/drivers/kvm2/",
		GOOS:     []string{"linux"},
		ExitCode: exitcode.DriverNotFound,
	},
	"KVM_CREATE_CONFLICT": {
...
// LocalNetworkErrors are errors communicating to the guest
var LocalNetworkErrors = map[string]match{
	"SSH_AUTH_FAILURE": {
		...
		URL:      vpnDoc,
		Issues:   []int{3930},
		ExitCode: exitcode.LocalNetworkConfig,
	},
	"SSH_TCP_FAILURE": {
	...
	"HOST_CIDR_CONFLICT": {
		...
		Issues:   []int{3594},
		ExitCode: exitcode.LocalNetworkConflict,
	},
	"VBOX_HOST_ADAPTER": {
		...
		ExitCode: exitcode.LocalNetworkNotFound,
	},
	"VBOX_IP_CONFLICT": {
	...
	"WAITING_FOR_SSH": {
		...
		Advice:   "Try 'minikube delete', and disable any conflicting VPN or firewall software",
		Issues:   []int{4617},
		ExitCode: exitcode.LocalNetworkTimeout,
	},

}


// InternetProblems are internet related problems.
var InternetProblems = map[string]match{
	"GCR_UNAVAILABLE": {
		...
		URL:      proxyDoc,
		Issues:   []int{3860},
		ExitCode: exitcode.InternetUnavailable,
	},
	...
	"DOWNLOAD_IO_TIMEOUT": {
		...
		URL:      proxyDoc,
		Issues:   []int{3846},
		ExitCode: exitcode.InternetTimeout,
	},
	"DOWNLOAD_TLS_OVERSIZED": {
		...
		ExitCode: exitcode.InternetConflict,
	},
	"DOWNLOAD_BLOCKED": {
	...
	"INVALID_PROXY_HOSTNAME": {
		...
		URL:      proxyDoc,
		ExitCode: exitcode.InternetConfig,
	},
	"HTTP_HTTPS_RESPONSE": {
...
var GuestErrors = map[string]match{
	...
	"NONE_PORT_IN_USE": {
		...
		Issues:   []int{5484},
		GOOS:     []string{"linux"},
		ExitCode: exitcode.GuestConflict,
	},
	"SYSTEMCTL_EXIT_1": {
	...
	"MACHINE_DOES_NOT_EXIST": {
		...
		ExitCode: exitcode.GuestMissing,
	},
	"MACHINE_NOT_FOUND": {
	...
	"IP_NOT_FOUND": {
		...
		ExitCode: exitcode.GuestNotRunning,
	},
...
// RuntimeErrors are container runtime issues (containerd, docker, etc)
var RuntimeErrors = map[string]match{
	...
	"DOCKER_UNAVAILABLE": {
		...
		Issues:   []int{3952},
		ExitCode: exitcode.RuntimeUnavailable,
	},	
	"NONE_DOCKER_EXIT_1": {
		...
		Issues:   []int{4498},
		GOOS:     []string{"linux"},
		ExitCode: exitcode.RuntimeNotFound,
	},
	"NONE_DOCKER_EXIT_5": {
...
// KubernetesErrors are Kubernetes deployment issues
var KubernetesErrors = map[string]match{
	"APISERVER_MISSING": {
		Regexp:   re(`apiserver process never appeared`),
		Advice:   "Check that the provided apiserver flags are valid, and that SELinux is disabled",
		Issues:   []int{4536, 6014},
		ExitCode: exitcode.KubernetesError,
	},
	"APISERVER_TIMEOUT": {
		...
		URL:      vpnDoc,
		Issues:   []int{4302},
		ExitCode: exitcode.KubernetesTimeout,
	},
	"DNS_TIMEOUT": {
	...
	"SERVICE_NOT_FOUND": {
		...
		Issues:   []int{4599},
		ExitCode: exitcode.KubernetesNotFound,
	},
