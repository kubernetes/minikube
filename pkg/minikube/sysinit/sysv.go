...
// SysV is a service manager for SysV-based init systems
type SysV struct {
	r Runner
}
...
func (s *SysV) Active(svc string) bool {
	_, err := s.r.RunCmd(exec.Command("sudo", path.Join("/etc/init.d", svc), "status"))
	return err == nil
}
...
func (s *SysV) Start(svc string) error {
	...
	_, err := s.r.RunCmd(exec.Command("sudo", path.Join("/etc/init.d", svc), "start"))
	return err
}

// Restart restarts a service
func (s *SysV) Restart(svc string) error {
	_, err := s.r.RunCmd(exec.Command("sudo", path.Join("/etc/init.d", svc), "restart"))
	return err
}

// Stop stops a service
func (s *SysV) Stop(svc string) error {
	_, err := s.r.RunCmd(exec.Command("sudo", path.Join("/etc/init.d", svc), "stop"))
	return err
}
