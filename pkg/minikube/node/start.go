...
func Provision(cc *config.ClusterConfig, n *config.Node, apiServer bool, delOnFail bool) (command.Runner, bool, libmachine.API, *host.Host, error) {
	...
	if apiServer {
		out.Step(style.ThumbsUp, "Starting control plane node {{.name}} in cluster {{.cluster}}", false, out.V{"name": name, "cluster": cc.Name})
	} else {
		out.Step(style.ThumbsUp, "Starting node {{.name}} in cluster {{.cluster}}", false, out.V{"name": name, "cluster": cc.Name})
	}
...
func validateNetwork(h *host.Host, r command.Runner, imageRepository string) (string, error) {
	...
	for _, k := range proxy.EnvVars {
		if v := os.Getenv(k); v != "" {
			if !optSeen {
				out.Step(style.Internet, "Found network options:", false)
				optSeen = true
			}
			...
			if (k == "HTTP_PROXY" || k == "HTTPS_PROXY") && !ipExcluded && !warnedOnce {
				...
				out.Step(style.Documentation, "Please see {{.documentation_url}} for more details", false, out.V{"documentation_url": "https://minikube.sigs.k8s.io/docs/handbook/vpn_and_proxy/"})
				warnedOnce = true
			}
		}
	}
...
func prepareNone() {
	...
	out.Step(style.StartingNone, "Configuring local host environment ...", false)
