...
func handleDownloadOnly(cacheGroup, kicGroup *errgroup.Group, k8sVersion string) {
	...
	out.Step(style.Check, false, "Download complete!")
	os.Exit(0)
}
...
func beginDownloadKicBaseImage(g *errgroup.Group, cc *config.ClusterConfig, downloadOnly bool) {
	...
	out.Step(style.Pulling, false, "Pulling base image ...")
	g.Go(func() error {
	...
	if err := g.Wait(); err != nil {
		if err != nil {
			if errors.Is(err, image.ErrGithubNeedsLogin) {
				...
				out.Step(style.Documentation, false, `Please visit the following link for documentation around this: 
