...
func StopHost(api libmachine.API, machineName string) error {
	...
	out.Step(style.Stopping, `Stopping node "{{.name}}"  ...`, false, out.V{"name": machineName})
	return stop(h)
}
...
func trySSHPowerOff(h *host.Host) error {
	...
	out.Step(style.Shutdown, `Powering off "{{.profile_name}}" via SSH ...`, false, out.V{"profile_name": h.Name})
