...
func CacheAndLoadImages(images []string, profiles []*config.Profile, overwrite bool, options *run.Options) error {
	if len(images) == 0 {
		return nil
	}
...
func DoLoadImages(images []string, profiles []*config.Profile, cacheDir string, overwrite bool, options *run.Options) error {
...
func SaveAndCacheImages(images []string, profiles []*config.Profile, options *run.Options) error {
	if len(images) == 0 {
		return nil
	}
...
func DoSaveImages(images []string, output string, profiles []*config.Profile, cacheDir string, options *run.Options) error {
	api, err := NewAPIClient(options)
	if err != nil {
		return errors.Wrap(err, "api")
	}
...
func PullImages(images []string, profile *config.Profile, options *run.Options) error {
	api, err := NewAPIClient(options)
	if err != nil {
...
func RemoveImages(images []string, profile *config.Profile, options *run.Options) error {
	api, err := NewAPIClient(options)
	if err != nil {
		return errors.Wrap(err, "error creating api client")
	}
...
func ListImages(profile *config.Profile, format string, options *run.Options) error {
	api, err := NewAPIClient(options)
	if err != nil {
		return errors.Wrap(err, "error creating api client")
	}
...
func TagImage(profile *config.Profile, source string, target string, options *run.Options) error {
	api, err := NewAPIClient(options)
	if err != nil {
		return errors.Wrap(err, "error creating api client")
	}
...
func PushImages(images []string, profile *config.Profile, options *run.Options) error {
	api, err := NewAPIClient(options)
	if err != nil {
		return errors.Wrap(err, "error creating api client")
	}
