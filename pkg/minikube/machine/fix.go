...
func recreateIfNeeded(api libmachine.API, cc *config.ClusterConfig, n *config.Node, h *host.Host) (*host.Host, error) {
	...
	if serr != nil || s == state.Stopped || s == state.None {
		...
		if !me || err == constants.ErrMachineMissing {
			out.Step(style.Shrug, out.NoSpinner, `{{.driver_name}} "{{.cluster}}" {{.machine_type}} is missing, will recreate.`, out.V{"driver_name": cc.Driver, "cluster": machineName, "machine_type": machineType})
			demolish(api, *cc, *n, h)
	...
	if s == state.Running {
		if !recreated {
			out.Step(style.Running, out.NoSpinner, `Updating the running {{.driver_name}} "{{.cluster}}" {{.machine_type}} ...`, out.V{"driver_name": cc.Driver, "cluster": machineName, "machine_type": machineType})
		}
		return h, nil
	}

	if !recreated {
		out.Step(style.Restarting, out.NoSpinner, `Restarting existing {{.driver_name}} {{.machine_type}} for "{{.cluster}}" ...`, out.V{"driver_name": cc.Driver, "cluster": machineName, "machine_type": machineType})
	}
...
func maybeWarnAboutEvalEnv(drver string, name string) {
	...
	if os.Getenv(constants.MinikubeActiveDockerdEnv) != "" {
		out.Step(style.Notice, out.NoSpinner, "Noticed you have an activated docker-env on {{.driver_name}} driver in this terminal:", out.V{"driver_name": drver})
	...
	if os.Getenv(constants.MinikubeActivePodmanEnv) != "" {
		out.Step(style.Notice, out.NoSpinner, "Noticed you have an activated podman-env on {{.driver_name}} driver in this terminal:", out.V{"driver_name": drver})
