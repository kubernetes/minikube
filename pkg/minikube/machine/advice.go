...
func MaybeDisplayAdvice(err error, driver string) {
	...
	if errors.Is(err, oci.ErrExitedUnexpectedly) || errors.Is(err, oci.ErrDaemonInfo) {
		out.Step(style.Tip, "If you are still interested to make {{.driver_name}} driver work. The following suggestions might help you get passed this issue:", false, out.V{"driver_name": driver})
		if driver == oci.Docker || driver == oci.Podman {
			...
			out.Step(style.Empty, `- Prune unused {{.driver_name}} images, volumes, networks and abandoned containers.
				...
				{{.driver_name}} system prune --volumes`, false, out.V{"driver_name": driver})
		}
		out.String("\n\t", false)
		out.Step(style.Empty, `- Restart your {{.driver_name}} service`, false, out.V{"driver_name": driver})
		if runtime.GOOS != "linux" {
			out.String("\n\t", false)
			out.Step(style.Empty, `- Ensure your {{.driver_name}} daemon has access to enough CPU/memory resources.`, false, out.V{"driver_name": driver})
			if runtime.GOOS == "darwin" && driver == oci.Docker {
				out.String("\n\t", false)
				out.Step(style.Empty, `- Docs https://docs.docker.com/docker-for-mac/#resources`, false, out.V{"driver_name": driver})
			}
			if runtime.GOOS == "windows" && driver == oci.Docker {
				out.String("\n\t", false)
				out.Step(style.Empty, `- Docs https://docs.docker.com/docker-for-windows/#resources`, false, out.V{"driver_name": driver})
			}
		}
		out.String("\n\t", false)
		out.Step(style.Empty, `- Delete and recreate minikube cluster
		minikube delete
		minikube start --driver={{.driver_name}}`, false, out.V{"driver_name": driver})
