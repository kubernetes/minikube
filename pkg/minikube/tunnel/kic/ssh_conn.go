...
func createSSHConn(name, sshPort, sshKey string, svc *v1.Service) *sshConn {
	...
	if askForSudo {
		...
		out.Step(style.Permissions, out.NoSpinner, "sudo permission will be asked for it.")
...
func (c *sshConn) startAndWait() error {
	out.Step(style.Running, out.NoSpinner, "Starting tunnel for service {{.service}}.", out.V{"service": c.service})
...
func (c *sshConn) stop() error {
	out.Step(style.Stopping, out.NoSpinner, "Stopping tunnel for service {{.service}}.", out.V{"service": c.service})
