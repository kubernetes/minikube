...
func TestPersistentRegistryWithNoKey(t *testing.T) {
	...
	if err == nil {
		t.Errorf("attempting to register TunnelInfo without key should throw error")
	}
}
...
func TestPersistentRegistryOperations(t *testing.T) {
	tcs := []struct {
		...
		},
		{
			...
			fileName: "nonexistent.txt",
			test: func(t *testing.T, reg *persistentRegistry) {
				e := reg.Remove(parseRoute("1.2.3.4", "1.2.3.4/5"))
				if e == nil {
		...
		},
		{
			...
			fileName: "nonexistent.txt",
			test: func(t *testing.T, reg *persistentRegistry) {
				e := reg.Register(&TunnelInfo{Route: parseRoute("1.2.3.4", "1.2.3.4/5")})
				if e != nil {
		...
		},
		{
			...
			test: func(t *testing.T, reg *persistentRegistry) {
				...
				e = reg.Remove(parseRoute("5.6.7.8", "1.2.3.4/5"))
				if e == nil {
		...
		},
		{
			...
			test: func(t *testing.T, reg *persistentRegistry) {
				e := reg.Register(&TunnelInfo{
					Route:       parseRoute("192.168.1.25", "10.96.0.0/12"),
					MachineName: "testmachine",
					Pid:         1234,
				})
				if e != nil {
				...
				e = reg.Remove(parseRoute("192.168.1.25", "10.96.0.0/12"))
