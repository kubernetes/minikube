...
func TestGlobalAvailable(t *testing.T) {
	...
	bar := DriverDef{
		...
		Default:  true,
		Priority: Default,
		Status:   func(_ run.Options) State { return State{Healthy: true} },
	}
	...
	foo := DriverDef{
		...
		Default:  true,
		Priority: Default,
		Status:   func(_ run.Options) State { return State{Healthy: false} },
	}
	if err := Register(foo); err != nil {
		t.Errorf("register returned error: %v", err)
	}
	...
	if diff := cmp.Diff(Available(false, run.Options{}), expected); diff != "" {
...
func TestGlobalStatus(t *testing.T) {
	...
	bar := DriverDef{
		...
		Default:  true,
		Priority: Default,
		Status:   func(_ run.Options) State { return expected },
	}
	if err := Register(bar); err != nil {
		t.Errorf("register returned error: %v", err)
	}

	if diff := cmp.Diff(Status("bar", run.Options{}), expected); diff != "" {
