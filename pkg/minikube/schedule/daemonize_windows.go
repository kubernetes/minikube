...
func KillExisting(profiles []string, options *run.Options) {
...
func killExisting(profile string, options *run.Options) error {
...
func daemonize(profiles []string, duration time.Duration, options *run.Options) error {
	for _, profile := range profiles {
...
// requested duration
func startSystemdService(profile string, duration time.Duration, options *run.Options) error {
	// get ssh runner
