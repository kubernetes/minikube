...
func Partial(name string, miniHome ...string) (libmachine.API, *config.ClusterConfig) {
	...
	if err != nil {
		if config.IsNotExist(err) {
			out.Step(style.Shrug, out.NoSpinner, `Profile "{{.cluster}}" not found. Run "minikube profile list" to view all profiles.`, out.V{"cluster": name})
...
func Running(name string) ClusterController {
	...
	if hs == state.None.String() {
		out.Step(style.Shrug, out.NoSpinner, `The control plane node "{{.name}}" does not exist.`, out.V{"name": cp.Name})
		exitTip("start", name, reason.ExGuestNotFound)
	}

	if hs == state.Stopped.String() {
		out.Step(style.Shrug, out.NoSpinner, `The control plane node must be running for this command`)
		exitTip("start", name, reason.ExGuestUnavailable)
	}

	if hs != state.Running.String() {
		out.Step(style.Shrug, out.NoSpinner, `The control plane node is not running (state={{.state}})`, out.V{"name": cp.Name, "state": hs})
...
func Healthy(name string) ClusterController {
	...
	if as == state.Paused {
		out.Step(style.Shrug, out.NoSpinner, `The control plane for "{{.name}}" is paused!`, out.V{"name": name})
		exitTip("unpause", name, reason.ExSvcConfig)
	}

	if as != state.Running {
		out.Step(style.Shrug, out.NoSpinner, `This control plane is not running! (state={{.state}})`, out.V{"state": as.String()})
...
func exitTip(action string, profile string, code int) {
	...
	out.Step(style.Workaround, out.NoSpinner, `To start a cluster, run: "{{.command}}"`, out.V{"command": command})
	os.Exit(code)
}
