...
// MachineConfigV162 contains the parameters used to start a cluster.
type MachineConfigV162 struct {
	Name                string
	KeepContext         bool // used by start and profile command to or not to switch kubectl's current context
	EmbedCerts          bool // used by kubeconfig.Setup
	MinikubeISO         string
...
// KubernetesConfigV162 contains the parameters used to configure the VM Kubernetes.
type KubernetesConfigV162 struct {
	KubernetesVersion string
	NodeIP            string
	NodePort          int
	NodeName          string
