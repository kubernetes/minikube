...
func OutputProblems(problems map[string][]string, maxLines int) {
	for name, lines := range problems {
		...
		for _, l := range lines {
			out.Step(style.LogEntry, out.NoSpinner, l)
		}
	}
}
...
func Output(r cruntime.Manager, bs bootstrapper.Bootstrapper, cfg config.ClusterConfig, runner command.Runner, lines int) error {
	...
	for i, name := range names {
		if i > 0 {
			out.Step(style.Empty, out.NoSpinner, "")
		}
		out.Step(style.Empty, out.NoSpinner, "==> {{.name}} <==", out.V{"name": name})
		var b bytes.Buffer
		c := exec.Command("/bin/bash", "-c", cmds[name])
		c.Stdout = &b
		c.Stderr = &b
		...
		scanner := bufio.NewScanner(&b)
		for scanner.Scan() {
			out.Step(style.Empty, out.NoSpinner, scanner.Text())
		}
	}
