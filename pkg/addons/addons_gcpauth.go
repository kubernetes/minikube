...
func enableOrDisableGCPAuth(cfg *config.ClusterConfig, name, val string, options *run.Options) error {
...
func enableAddonGCPAuth(cfg *config.ClusterConfig, options *run.Options) error {
...
func disableAddonGCPAuth(cfg *config.ClusterConfig, options *run.Options) error {
	// Grab command runner from running cluster
	cc := mustload.Running(cfg.Name, options)
	r := cc.CP.Runner
...
func verifyGCPAuthAddon(cc *config.ClusterConfig, name, val string, options *run.Options) error {
	enable, err := strconv.ParseBool(val)
	if err != nil {
		return errors.Wrapf(err, "parsing bool: %s", name)
	}
