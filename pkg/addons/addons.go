...
func RunCallbacks(cc *config.ClusterConfig, name string, value string, options *run.Options) error {
...
func Set(cc *config.ClusterConfig, name string, value string, options *run.Options) error {
	a, valid := isAddonValid(name)
	if !valid {
		return errors.Errorf("%s is not a valid addon", name)
	}
...
func SetAndSave(profile string, name string, value string, options *run.Options) error {
...
func invoke(cc *config.ClusterConfig, name string, value string, fns []setFn, options *run.Options) error {
	var errs []error
	for _, fn := range fns {
...
func SetBool(cc *config.ClusterConfig, name string, val string, _ *run.Options) error {
...
func EnableOrDisableAddon(cc *config.ClusterConfig, name string, val string, options *run.Options) error {
...
func verifyAddonStatus(cc *config.ClusterConfig, name string, val string, options *run.Options) error {
	ns := "kube-system"
	if name == "ingress" {
		ns = "ingress-nginx"
	}
...
func verifyAddonStatusInternal(cc *config.ClusterConfig, name string, val string, ns string, _ *run.Options) error {
...
func Enable(wg *sync.WaitGroup, cc *config.ClusterConfig, toEnable map[string]bool, enabled chan<- []string, options *run.Options) {
	defer wg.Done()
...
func UpdateConfigToEnable(cc *config.ClusterConfig, enabled []string, options *run.Options) {
...
func UpdateConfigToDisable(cc *config.ClusterConfig, options *run.Options) {
...
func VerifyNotPaused(profile string, enable bool, options *run.Options) error {
