...
func dockerImagesPreloaded(runner runner.Runner, images []string) bool {
...
func dockerBoundToContainerd(runner runner.Runner) bool {
...
func getCriDockerdPath(cr runner.Runner) string {
...
func generateCRIDockerdConfig(cr runner.Runner, imageRepository string, kv semver.Version, networkPlugin string) error {
...
ExecStart={{.ExecPath}} --container-runtime-endpoint fd:// --pod-infra-container-image={{.PauseImage}} --network-plugin={{.NetworkPlugin}}{{.ExtraArguments}}`))
	...
	if err := cr.CopyFile(svc); err != nil {
