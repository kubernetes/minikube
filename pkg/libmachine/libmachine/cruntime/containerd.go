...
func generateContainerdConfig(cr runner.Runner, imageRepository string, kv semver.Version, cgroupDriver string, insecureRegistry []string, inUserNamespace bool) error {
...
func gitClone(cr runner.Runner, src string) (string, error) {
...
func downloadRemote(cr runner.Runner, src string) (string, error) {
	u, err := url.Parse(src)
	if err != nil {
		return "", err
	}
...
func (r *Containerd) Preload(cc config.ClusterConfig) error {
	...
	if err := r.Runner.RemoveFile(fa); err != nil {
...
func containerdImagesPreloaded(runner runner.Runner, images []string) bool {
