...
func (s Filestore) Save(machine *machine.Machine) error {
	data, err := json.MarshalIndent(machine, "", "    ")
	if err != nil {
		return err
	}

	hostPath := filepath.Join(s.GetMachinesDir(), machine.Name)
...
func (s Filestore) loadConfig(m *machine.Machine) error {
	data, err := ioutil.ReadFile(filepath.Join(s.GetMachinesDir(), m.Name, "config.json"))
	if err != nil {
		return err
	}
	...
	migratedHost, migrationPerformed, err := machine.MigrateHost(m, data)
	if err != nil {
	...
	*m = *migratedHost
	...
	if migrationPerformed {
		if err := s.saveToFile(data, filepath.Join(s.GetMachinesDir(), m.Name, "config.json.bak")); err != nil {
...
func (s Filestore) Load(name string) (*machine.Machine, error) {
	hostPath := filepath.Join(s.GetMachinesDir(), name)
