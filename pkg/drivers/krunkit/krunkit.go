...
// Driver is the machine driver for krunkit (Virtualization.framework)
type Driver struct {
	*drivers.BaseDriver
...
// GetState returns driver state. Since krunkit driver may use 2 processes
...
func (d *Driver) ResolveStorePath(filename string) string {
	return filepath.Join(d.StorePath, "machines", d.GetMachineName(), filename)
}
...
func (d *Driver) stopKrunkit() error {
	if err := d.SetKrunkititState("Stop"); err != nil {
		// krunkit may be already stopped, shutting down, or not listening.
...
func (d *Driver) GetkrunkitState() (string, error) {
	...
	log.Debugf("get krunkit state: %+v", vmstate)
