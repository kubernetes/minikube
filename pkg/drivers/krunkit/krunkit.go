...
func (d *Driver) stopKrunkit() error {
	// krunkit supports stopping via HTTP like vfkit, but this does not work
	// with minikube VM with kernel 6.6. It works with Fedora 42 VM.
	// TODO: When fixed, used HTTP API for termination.
	pidfile := d.pidfilePath()
	pid, err := process.ReadPidfile(pidfile)
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
