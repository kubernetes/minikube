...
// addr parses a literal address for test tables; invalid literals panic the
// test with a clear zero Addr mismatch, which never happens for the curated
// table values below.
...
func TestValidateSemantics_Valid(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "192.168.200.0/24 (maintainer example)",
		...
		{
			name:  "172.16.0.0/24 inside 172.16/12",
		...
		{
			name:  "192.168.0.0/24 with end two below broadcast",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := validateSemantics(addr(tt.start), addr(tt.end), addr(tt.mask)); err != nil {
...
func TestValidateSemantics_Invalid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateSemantics(addr(tt.start), addr(tt.end), addr(tt.mask))
			if err == nil {
...
func TestNormalizeAddress_Valid(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if got.String() != tt.want || !got.IsValid() {
			t.Errorf("NormalizeAddress(%q) = %v, want %q", tt.val, got, tt.want)
		}
	}
}
...
func TestNormalizeAddress_Invalid(t *testing.T) {
	tests := []struct {
		...
		{val: "", errMatch: "not a valid IPv4"},                   // empty is not a value
...
func TestNormalizeSubnetMask_Valid(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if got.String() != tt.want {
			t.Errorf("NormalizeSubnetMask(%q) = %v, want %q", tt.val, got, tt.want)
		}
	}
}
...
func TestIsValidVmnetAddress_Invalid(t *testing.T) {
	tests := []struct {
		...
		{val: "", errMatch: "not a valid IPv4"},              // empty is not storable; `config unset` clears a key
		{val: "8.8.8.8", errMatch: "RFC 1918"},               // public
		{val: "100.64.0.1", errMatch: "RFC 1918"},            // CGN (not IsPrivate)
		{val: "::1", errMatch: "not a valid IPv4"},           // IPv6
		{val: "2001:db8::1", errMatch: "not a valid IPv4"},   // IPv6
...
func TestIsValidVmnetSubnetMask_Invalid(t *testing.T) {
	tests := []struct {
		...
		{val: "", errMatch: "not a valid IPv4"},                // empty is not storable
		{val: "255.255.255.1", errMatch: "contiguous"},         // non-contiguous
		{val: "255.0.255.0", errMatch: "contiguous"},           // non-contiguous
		{val: "::1", errMatch: "not a valid IPv4"},             // IPv6
		{val: "255.255.255.256", errMatch: "not a valid IPv4"}, // bad octet
...
func TestAddrHelpers(t *testing.T) {
	...
	if p := AddrPtr(zero); p != nil {
		t.Errorf("AddrPtr(zero) = %v, want nil", p)
	}
	if p := AddrPtr(set); p == nil || p.String() != "192.168.1.1" {
		t.Errorf("AddrPtr(set) = %v, want pointer to 192.168.1.1", p)
	}

	if got := AddrOrZero(nil); got.IsValid() {
		t.Errorf("AddrOrZero(nil) = %v, want zero Addr", got)
	}
	if got := AddrOrZero(&set); got != set {
		t.Errorf("AddrOrZero(&set) = %v, want %v", got, set)
	}

	if got := AddrString(nil); got != "" {
		t.Errorf("AddrString(nil) = %q, want \"\"", got)
	}
	if got := AddrString(&zero); got != "" {
		t.Errorf("AddrString(&zero) = %q, want \"\"", got)
	}
	if got := AddrString(&set); got != "192.168.1.1" {
		t.Errorf("AddrString(&set) = %q, want %q", got, "192.168.1.1")
	}
}
