...
// mustAddr parses a literal address for test tables.
...
func TestHelperValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "valid triple",
			helper: Helper{
				StartAddress: mustAddr("192.168.200.1"),
				EndAddress:   mustAddr("192.168.200.127"),
				SubnetMask:   mustAddr("255.255.255.0"),
			},
		},
		{
			name: "partial triple rejected",
			helper: Helper{
				StartAddress: mustAddr("192.168.1.1"),
			},
			match: "set together",
		},
		{
			...
			helper: Helper{
				StartAddress: mustAddr("192.168.1.1"),
				EndAddress:   mustAddr("192.168.2.10"),
				SubnetMask:   mustAddr("255.255.255.0"),
			},
			match: "same subnet",
		},
		{
			...
			helper: Helper{
				StartAddress: mustAddr("192.168.1.10"),
				EndAddress:   mustAddr("192.168.1.5"),
				SubnetMask:   mustAddr("255.255.255.0"),
			},
			match: "greater than",
		},
